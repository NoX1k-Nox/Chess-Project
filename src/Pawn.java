public class Pawn extends ChessPiece {
    ...
    protected boolean canMoveSpecific(ChessBoard chessBoard, int line, int column, int toLine, int toColumn) {
        ...
        if (column == toColumn) {
            ...
            if (line == startLine && moveDistance <= 2 && (toLine - line) * direction > 0) {
                return moveDistance != 2 || chessBoard.board[line + direction][column] == null;
            }
            return moveDistance == 1 && (toLine - line) * direction > 0;
        } else if (Math.abs(column - toColumn) == 1 && moveDistance == 1 && (toLine - line) * direction > 0) {
